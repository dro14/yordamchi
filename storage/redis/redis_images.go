...
func (r *Redis) Images(ctx context.Context) int {
	images, _ := client.Get(ctx, "images:"+id(ctx)).Int()
	return images
}

func (r *Redis) DecrementImages(ctx context.Context) {
	images, err := client.Get(ctx, "images:"+id(ctx)).Int()
	if err != nil {
	...
	if images > 1 {
		client.Set(ctx, "images:"+id(ctx), images-1, 0)
	} else if images == 1 {
		client.Del(ctx, "images:"+id(ctx))
	} else {
...
func (r *Redis) Generate(ctx context.Context) string {
	generate, err := client.Get(ctx, "generate:"+id(ctx)).Result()
	if err != nil {
		log.Printf("can't get %q: %s", "generate:"+id(ctx), err)
		return ""
	}
	client.Del(ctx, "generate:"+id(ctx))
	return generate
}

func (r *Redis) SetGenerate(ctx context.Context, generate string) {
	client.Set(ctx, "generate:"+id(ctx), generate, 0)
}

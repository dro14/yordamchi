...
func (p *Postgres) UserStarted(ctx context.Context, user *tgbotapi.User) {
	...
	query += "UPDATE users SET first_name = $8, last_name = $9, username = $10, language_code = $11, is_active = true WHERE id = $12;"
	args = append(args, user.FirstName, user.LastName, user.UserName, lang(ctx), user.ID)
...
func (p *Postgres) SaveMessage(ctx context.Context, user *tgbotapi.User, msg *Message) {
	...
	query += "UPDATE users SET first_name = $17, last_name = $18, username = $19, language_code = $20, is_active = true WHERE id = $21;"
	args = append(args, user.FirstName, user.LastName, user.UserName, lang(ctx), user.ID)
	err := p.execTelegram(ctx, user, query, args)
	if err != nil {
